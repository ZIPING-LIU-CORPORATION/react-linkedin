...
export const LIRenderAll = (props: Partial<LIRenderAllProps>) => {
  ...
  const childScripts: LIRenderAllProps["childScripts"] = React.useMemo(() => props.childScripts || {}, [props.childScripts]);


  // These functions are needed because badge markup is added via innerHtml property which does not run script tags
  const shouldReplaceNode = React.useCallback(
    (
      node: { src: string | number; getAttribute: (name: string) => string | null } & {
        tagName: string;
      },
      isCreate = 'false',
    ) => {
      return (
        ...
        (!(isCreate === 'true')
          || ((isCreate === 'true'
          ) && !node.getAttribute("data-isartdeco")))
      );
    },
    [childScripts],
  );

  const logDegug = React.useCallback((message: string, type: string, componentName: string) => {
    if (props.debug) {
      const currentTime = new Date().toLocaleTimeString();
      const componentNameBoldedOnly = `%c${componentName}`;
      const messageNormal = `%c${message}`;
      const typeColored = `%c${type}`;
      console.log(`[${currentTime}] ${typeColored
        } -  ${componentNameBoldedOnly}: ${messageNormal}`, "color:skyblue", "font-weight: bold", "color: green");
    }
  }, [props.debug]);
  ...
  const responseHandler = React.useCallback(
    (badgeHtml: string, badgeUid: number) => {
      ...
      for (const badge of badges || []) {
        ...
        if (uid === badgeUid) {
          const badgeMarkup = `<body>${badgeHtml
            }</body>`;
...
function generateUrl(isEI: boolean) {
